...
WITH params AS (
    ...
    SELECT
        ...
        toDate('2026-04-05') AS detection_to,      -- window LEBAR (akhir)
        toDate('2026-04-04') AS target_from,       -- tanggal spike yang mau di-cek / DELETE
        toDate('2026-04-04') AS target_to,         -- tanggal spike yang mau di-cek / DELETE
        'Shopee'             AS channel,
        'Parfum Unisex'      AS l3title,           -- filter L3Title, isi '' untuk semua
...
--    channel          : 'Shopee'
--    l3title          : 'Parfum Unisex'
--    target_from/to   : '2026-04-04'
--    detection window : '2026-03-28' s/d '2026-04-05'
--    spike_jump_mult  : 5
-- ============================================================
ALTER TABLE default.dm_Mandom
DELETE WHERE
    Channel   = 'Shopee'
    AND ScrapDate BETWEEN '2026-04-04' AND '2026-04-04'
    AND DailySalesValue > 0
    AND L3Title = 'Parfum Unisex'
    AND ItemId IN (
        SELECT ItemId FROM (
            ...
            FROM (
                ...
                WHERE Channel   = 'Shopee'
                  AND L3Title   = 'Parfum Unisex'
                  AND ScrapDate BETWEEN '2026-03-28' AND '2026-04-05'
            ...
            INNER JOIN (
                ...
                WHERE Channel   = 'Shopee'
                  AND L3Title   = 'Parfum Unisex'
                  AND ScrapDate BETWEEN '2026-03-28' AND '2026-04-05'
                  AND (ScrapDate < '2026-04-04' OR ScrapDate > '2026-04-04')
                  AND DailySalesCount > 0
                GROUP BY ItemId, Channel
